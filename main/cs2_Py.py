...
# Константы
CONFIG_DIR = os.path.join(os.environ['LOCALAPPDATA'], 'temp', 'Syfer-eng')
...
def get_offsets_and_client_dll():
    offsets = requests.get('https://raw.githubusercontent.com/SYFER-eng/Cs2.exe_Cheat/refs/heads/main/main/output/offsets.json').json()
    client_dll = requests.get('https://raw.githubusercontent.com/SYFER-eng/Cs2.exe_Cheat/refs/heads/main/main/output/client_dll.json').json()
...
def get_window_size(window_title):
    ...
    if hwnd:
        ...
        return rect[2] - rect[0], rect[3] - rect[1]  # Calculate width and height
    return None, None
...
def create_checkbox(self, label, setting_key):
    ...
    checkbox.setChecked(self.settings[setting_key] == 1)
    checkbox.stateChanged.connect(self.save_settings)
    return checkbox

def create_combobox(self, label, items, setting_key):
    """Helper function to create combo boxes."""
    combobox = QtWidgets.QComboBox()
    combobox.addItems(items)
    combobox.setCurrentIndex(self.settings[setting_key])
    combobox.setStyleSheet("background-color: #020203; border-radius: 5px;")
    combobox.currentIndexChanged.connect(self.save_settings)
    return combobox

def create_line_edit(self, label, setting_key):
    """Helper function to create a text input field."""
    line_edit = QtWidgets.QLineEdit()
    line_edit.setText(self.settings[setting_key])
    line_edit.textChanged.connect(self.save_settings)
    line_edit.setStyleSheet("background-color: #020203; border-radius: 5px;")
    return line_edit
...
def create_esp_container(self):
    ...
    # Create checkboxes and comboboxes for ESP settings
    esp_layout.addWidget(self.create_checkbox("Enable ESP", "esp_rendering"))
    esp_layout.addWidget(self.create_combobox("ESP Mode", ["Enemies Only", "All Players"], "esp_mode"))
    esp_layout.addWidget(self.create_checkbox("Draw Lines", "line_rendering"))
    esp_layout.addWidget(self.create_checkbox("Draw HP Bars", "hp_bar_rendering"))
    esp_layout.addWidget(self.create_checkbox("Draw Head Hitbox", "head_hitbox_rendering"))
    esp_layout.addWidget(self.create_checkbox("Draw Bones", "bons"))
    esp_layout.addWidget(self.create_checkbox("Show Nickname", "nickname"))
    esp_layout.addWidget(self.create_checkbox("Show Weapon", "weapon"))
    esp_layout.addWidget(self.create_checkbox("Bomb ESP", "bomb_esp"))
...
def create_trigger_container(self):
    ...
    trigger_layout.addWidget(self.create_checkbox("Enable Trigger Bot", "trigger_bot_active"))
    trigger_layout.addWidget(QtWidgets.QLabel("Trigger Key:"))
    trigger_layout.addWidget(self.create_line_edit("Trigger Key", "keyboards"))
...
class ESPWindow(QtWidgets.QWidget):
    ...
    def update_scene(self):
        ...
        try:
            ...
            fps_text = self.scene.addText(f"Syfer-eng | FPS: {self.fps}", QtGui.QFont('DejaVu Sans', 12, QtGui.QFont.Bold))
            fps_text.setPos(5, 5)
            fps_text.setDefaultTextColor(QtGui.QColor(223, 115, 255))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            pass
    print("Starting Syfer-eng's cheat!")
    time.sleep(2)
